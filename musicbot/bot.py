...
#######################################################################################################################
    ...
    async def cmd_queue(self, channel, player):
        ...
        for i, item in enumerate(player.playlist, 1):
            time+=item.duration
            item_dur = ftimedelta(timedelta(seconds=item.duration))
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = self.str.get('cmd-queue-entry-author', '{0} -- **{1}** by **{2}** `{3}`').format(i, item.title, item.meta['author'].name, item_dur).strip()
            else:
                nextline = self.str.get('cmd-queue-entry-noauthor', '{0} -- **{1}** `{3}`').format(i, item.title, item_dur).strip()
